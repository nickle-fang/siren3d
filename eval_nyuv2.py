...
def eval_main_loop(dataloader, feature_ex_net, base_func):
    ...
    for i in range(eval_pics):
        with torch.no_grad():
            ...
            rmse_single = torch.square(predict_depth - disp_gt)
            rmse_single = torch.sum(rmse_single) / rmse_single.numel()
            total_rmse += math.sqrt(rmse_single.item())
            print("Single picture " + str(i) + " loss is :", rmse_single.item())
...
def eval_main():
    ...
    nyu_eval_dataset = nyu.NYUDataset(filelistpath="./data/NYUv2_all.txt",
