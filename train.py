    ...
    for epoch in range(total_epochs):
        ...
        for i in range(total_pics):
            ...
            for p in point_sample_index:
                fc_in = torch.zeros(1, 66)
                fc_in[:, :64] = feature_map[:, :, p[0], p[1]]
                fc_in[:, 64] = p[0]
                fc_in[:, 65] = p[1]
            ...
            depth_sample = depth_sample.cpu().squeeze().numpy()
            fc_concat_trans = np.transpose(fc_concat)

            new_a = np.dot(fc_concat_trans, fc_concat)
            ...
            feature_map = feature_map.reshape(1, 64, 224*224)
